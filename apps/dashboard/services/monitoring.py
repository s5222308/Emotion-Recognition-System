...
# Import global state from core (clean absolute import)
from core import system_logs, service_metrics, debug_images, SERVICES
...
class SystemMonitor:
    ...
    def __init__(self):
        ...
        # CRITICAL: DO NOT create local service_metrics - use global one from core
        # self.service_metrics is removed - we use the global imported one
        self.debug_images = []
    ...
    def _check_all_services(self):
        ...
        for service_id in DashboardConfig.SERVICES.keys():
            ...
            # CRITICAL: Update the GLOBAL service_metrics that API endpoints use
            service_metrics[service_id]['status'] = status
            service_metrics[service_id]['last_check'] = datetime.now()
            
            # Set uptime when service becomes healthy (first time only)
            if status == 'healthy' and service_metrics[service_id]['uptime'] is None:
                service_metrics[service_id]['uptime'] = datetime.now()
                print(f"[DEBUG] Setting initial uptime for {service_id}")
            # Clear uptime when service goes down
            elif status != 'healthy':
                if service_metrics[service_id]['uptime'] is not None:
                    print(f"[DEBUG] Clearing uptime for {service_id} (status: {status})")
                service_metrics[service_id]['uptime'] = None
    ...
    def _add_log(self, level, message):
        """Add log entry"""
        # Import global state to write to shared logs
    ...
    def get_logs(self, level_filter='ALL', limit=50):
        """Get filtered logs"""
        # Import global state to read from shared logs
        from core import system_logs
    ...
    def get_services_status(self):
        """Get current services status with uptime calculation - EXACT as original"""
        status_data = {}
        
        for service_id in DashboardConfig.SERVICES.keys():
            service = DashboardConfig.SERVICES[service_id]
            metrics = service_metrics[service_id]
            
            # Calculate uptime exactly as original
            uptime_seconds = 0
            uptime_display = None
            if metrics.get('uptime'):
                current_time = datetime.now()
                uptime_start = metrics['uptime']
                uptime_seconds = int((current_time - uptime_start).total_seconds())
                uptime_display = uptime_start.strftime('%a, %d %b %Y %H:%M:%S GMT')
                print(f"[DEBUG UPTIME] {service_id}: current={current_time}, start={uptime_start}, seconds={uptime_seconds}")
            
            status_data[service_id] = {
                'name': service['name'],
                'description': service.get('description', service['name']),  # Match original
                'status': metrics.get('status', 'unknown'),
                'uptime': uptime_display,
                'uptime_seconds': uptime_seconds,
                'requests': metrics.get('requests', 0),
                'errors': metrics.get('errors', 0),
                'last_health_check': metrics.get('last_check', datetime.now()).isoformat(),
                'base_url': f"http://localhost:{service['port']}",
                'url': service.get('health_url', f"http://localhost:{service['port']}/status")
            }
